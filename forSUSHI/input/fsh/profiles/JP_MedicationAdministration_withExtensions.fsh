...
Id: JP_MedicationAdministration
...
* extension[Requester] only JP_MedicationAdministration_Requester
* extension[Requester] ^sliceName = "Requester"
...
* dosage.id ^short = "Unique id for inter-element referencing　要素間参照用の一意のID"
* dosage.id ^definition = "Unique id for the element within a resource (for internal references). This may be any string value that does not contain spaces.\r\n\r\nリソース内の要素の一意のID（内部参照用）。これは、スペースを含まない任意の文字列値にすることができる。"
